...
import {
  ...
  History, Type, Flame, Facebook, Eye, Twitter, Tag, Quote, MessageCircle
} from "lucide-react";
...
const SingleArticle = ({ articleId, onInView, isActive = true }: { articleId: string; onInView?: (slug: string, title: string) => void; isActive?: boolean }) => {
  ...
  return (
    <>
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-background/85 backdrop-blur-xl border-t border-border px-4 pt-3 pb-[calc(0.75rem+env(safe-area-inset-bottom))] flex items-center justify-around shadow-[0_-4px_24px_rgba(0,0,0,0.05)]" hidden={!isActive}>
