...
async def update_source(db: AsyncIOMotorDatabase, source_id: str, data: SourceUpdate) -> Optional[dict]:
    ...
    if not update:
        doc = await Sources.find_one({"_id": source_id})
        if doc:
            doc["id"] = doc["_id"]
        return doc

    result = await Sources.find_one_and_update(
        {"_id": source_id},
        {"$set": update},
...
async def delete_source(db: AsyncIOMotorDatabase, source_id: str) -> bool:
    result = await Sources.delete_one({"_id": source_id})
...
async def get_source(db: AsyncIOMotorDatabase, source_id: str) -> Optional[dict]:
    ...
    if doc:
        doc["id"] = doc["_id"]
        doc["article_count"] = await Article_sources.count_documents({"source_id": source_id})
    return doc
...
async def list_sources(db: AsyncIOMotorDatabase, active_only: bool = False) -> list[dict]:
    ...
    if active_only:
        query["active"] = True
    cursor = Sources.find(query).sort("created_at", -1)
    docs = await cursor.to_list(length=1000)
    for d in docs:
        d["id"] = d["_id"]
        d["article_count"] = await Article_sources.count_documents({"source_id": d["id"]})
    return docs


async def set_last_fetched(db: AsyncIOMotorDatabase, source_id: str):
    await Sources.update_one(
        {"_id": source_id},
